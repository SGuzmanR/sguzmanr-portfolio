...
const FloatingDockDesktop = ({
  ...
  return (
    (<motion.div
      ...
      className={cn(
        "mx-auto hidden md:flex h-16 gap-4 items-end rounded-2xl bg-secondaryGrayDark px-4 pb-3 z-20",
        className
      )}>
      {items.map((item) => (
  ...
  return (
    (<Link href={href} target="_blank">
      <motion.div
        ...
        className="aspect-square rounded-full bg-secondaryGrayDark text-primaryWhite flex items-center justify-center relative">
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="px-2 py-0.5 whitespace-pre rounded-md bg-secondaryGrayDark text-primaryWhite absolute left-1/2 -translate-x-1/2 -top-8 w-fit text-xs">
              {title}
            </motion.div>
          )}
        </AnimatePresence>
