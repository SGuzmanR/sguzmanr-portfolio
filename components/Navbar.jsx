...
const Navbar = () => {
  ...
  useEffect(() => {
    tl.current = gsap.timeline({ paused: true })
      ...
      .to(".mobile-svg_path-1", {
        translateX: "40%",
        ease: "sine.inOut",
        duration: .3
      })
      .to(".mobile-svg_path-3", {
        translateX: "80%",
        ease: "sine.inOut",
        duration: .3
      })
  ...
  const handleMouseEnter = (i) => {
    gsap.to(navLinksRef.current[i].querySelector('.nav-link_underline'), {
      width: '100%',
      duration: 0.5,
      ease: 'sine.out',
    });
  ...
  return (
    <header ref={navContainer} className="absolute w-full h-auto">
      ...
      {/* Nav */}
      <nav id="nav" className="paddingX flex flex-row justify-between items-center py-6 w-full">
        <Link href="/" className="nav-link opacity-0">
          <svg
            width="80"
            height="100%"
        ...
        <div className="flex flex-row gap-10 items-center justify-center max-sm:hidden">
          <div className="flex flex-row gap-6 w-full h-auto">
            {navLinks.map((link, i) => (
              <Link 
                ...
                onMouseEnter={() => handleMouseEnter(i)}
                onMouseLeave={() => handleMouseLeave(i)}
                className="relative nav-link opacity-0 -translate-y-[100%] font-montserrat font-thin text-sm text-gray hover:text-white will-change-auto transition-colors duration-700"
              >
                <span>{link.name}</span>
                <span className="nav-link_underline block absolute bottom-0 left-0 w-0 h-[0.5px] bg-white"></span>
              </Link>
            ))}
          </div>
          ...
          <div className="flex flex-row gap-4">
            {socialLinks.map((link) => (
              <Link key={link.name} href={link.href} target="_blank">
                <svg className="fill-gray hover:fill-white transition-colors duration-700 scale-125 nav-link opacity-0 " viewBox="0 0 576 512" width={20} height={20}>
                  {link.src}
                </svg>
              </Link>
            ))}
        ...
        <div className="max-sm:flex items-center justify-center hidden cursor-pointer">
          <svg
            width="80"
            height="100%"
            viewBox="0 0 400 100"
