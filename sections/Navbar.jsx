...
const Navbar = () => {
  ...
  return (
    <header ref={container}>
      <nav className="fixed flex justify-between items-center top-0 left-0 w-[100vw] p-[2em] z-10 bg-black">
        <div>
          <Link href="/" className="text-white cursor-pointer uppercase text-[14px] font-medium leading-[100%] no-underline">SGuzmanR</Link>
        </div>

        <div onClick={handleOpenNav}>
          <p className="text-white cursor-pointer uppercase text-[14px] font-medium leading-[100%] no-underline">Menu</p>
        </div>
      </nav>

      <div id="navbar-overlay" className="fixed flex top-0 left-0 w-[100vw] h-[100vh] p-[2em] pt-16 bg-white z-20 gap-16">
        ...
        <div className="flex-2 flex items-end cursor-pointer max-[900px]:hidden" onClick={handleOpenNav}>
          <p className="text-[100px] leading-[70%] text-black stroke-white hover:scale-105 transition-all duration-500">&#x2715;</p>
        </div>
            ...
            {navLinks.map((link, i) => (
              <div id="navbar-link-item" className="w-max" key={i}>
                <div id="navbar-link-item-holder" className="relative max-[900px]:text-[60px]" onClick={handleOpenNav}>
                  <Link href={link.href} className="flex text-black text-[80px] max-sm:text-[40px] font-normal tracking-[-0.02em] leading-[85%] uppercase hover:underline">
                    {link.name}
                  </Link>
                </div>
              </div>
